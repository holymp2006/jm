 ...
 *}
 ...
 <form method="post" action="{url op="saveSettings"}">
 	...
 	<div id="enableRT"><input type="checkbox" {if $enabled}checked="checked" {/if}name="enabled" value="1" id="enabled"/>&nbsp;&nbsp;<label for="enabled">{translate key="rt.admin.settings.enableReadingTools"}</label></div>
 	<hr>
 	...
 	<div id="rtAdminRelatedItems">
 		...
 		<div class="form-horizontal mb-15">
 			<div class="form-group">
 				<label for="version" class="col-sm-2 control-label">{translate key="rt.admin.settings.relatedItems"}</label>
 				<div class="col-sm-10">
 		...
 		{translate key="rt.admin.settings.relatedItemsLink" relatedItemsLink=$relatedItemsLink}

 	</div>
 	<hr>
 	<p><input type="submit" value="{translate key="common.save"}" class="btn btn-primary" /> <input type="button" value="{translate key="common.cancel"}" class="btn btn-warning" onclick="document.location.href='{url page="rtadmin" escape=false}'" /></p>
