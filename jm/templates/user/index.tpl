...
{if $isSiteAdmin}
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<a href="{url journal="index" page=$isSiteAdmin->getRolePath()}">
			<h4 class="panel-title"><strong><span><i class="fa fa-user"></i></span> {translate key=$isSiteAdmin->getRoleName()}</strong></h4>
		</div>
			<p class="panel-body" style="color: black;">Click here to manage site settings, hosted journals, languages, system information, cache and logs</p>
		</a>
...
<div id="myJournals" class="panel panel-default">
{if !$currentJournal}<div class="panel-heading"><h3><span><i class="fa fa-book"></i> {translate key="user.myJournals"}</span></h3></div>{/if}

{foreach from=$userJournals item=journal}
	<div id="journal-{$journal->getPath()|escape}" class="panel-body">
	{assign var="hasRole" value=1}
	{if !$currentJournal}<h4><a href="{url journal=$journal->getPath() page="user"}"><span><i class="glyphicon glyphicon-paperclip"></i>&nbsp; {$journal->getLocalizedTitle()|escape}</span></a></h4>
	{else}<h3><span><i class="glyphicon glyphicon-paperclip"></i>&nbsp;{$journal->getLocalizedTitle()|escape}</span></h3>{/if}
	{assign var="journalId" value=$journal->getId()}
	{assign var="journalPath" value=$journal->getPath()}
	<table width="100%" class="info table-responsive">
		{if $isValid.JournalManager.$journalId}
			<tr>
					<div>
						<a href="{url journal=$journalPath page="manager"}">
							<strong class="btn btn-default"><span><i class="fa fa-wrench"></i></span> {translate key="user.role.manager"}</strong>
						</a>
						<div class="btn btn-warning pull-right">{if $setupIncomplete.$journalId} <a href="{url journal=$journalPath page="manager" op="setup" path="1"}" style="color: white;" data-toggle="popover" title="Example popover" data-content="Some content in Popover on bottom" data-container="body" data-placement="bottom" data-animation="animation"> <span><i class="glyphicon glyphicon-pushpin"></i> {translate key="manager.setup"}</span></a>{/if} </div>

					</div>
<!-- <div>We've noticed that your journal setup hasn't been done yet or is incomplete. Click here to finish off the process. It'll only take a few minutes.</div> -->	
			</tr>
			<p></p>
		...
		{if $isValid.Editor.$journalId}
			<tr>
				...
				<td align="right" class="btn-group pull-right">
					<a href="{url journal=$journalPath page="editor" op="createIssue"}" class="btn btn-default">{translate key="editor.issues.createIssue"}</a> <a href="{url journal=$journalPath page="editor" op="notifyUsers"}" class="btn btn-default">{translate key="editor.notifyUsers"}</a>
					</td>
			</tr>
		{/if}
		...
		{if $isValid.Author.$journalId}
			...
			<tr>
				...
				<td align="right"><a href="{url journal=$journalPath page="author" op="submit"}" class="btn btn-default">{translate key="author.submit"}</a></td>
			</tr>
		{/if}
...
<div id="myAccount" class="panel panel-info">
<div class="panel-heading">
	<h3 class="panel-title"><a data-toggle="collapse" data-parent="#accordion" href="#collapseMyAccount"><span><i class="glyphicon glyphicon-option-vertical"></i></span> {translate key="user.myAccount"}</a></h3>
</div>

<div id="collapseMyAccount" class="panel-collapse collapse"> <div class="panel-body">
<ul class="list-group">
	{if $hasOtherJournals}
		{if !$showAllJournals}
			<li class="list-group-item-heading"><a href="{url journal="index" page="user"}"><span><i class="fa fa-book"></i></span> {translate key="user.showAllJournals"}</a></li>
		{/if}
	{/if}
	{if $currentJournal}
		{if $subscriptionsEnabled}
			<li class="list-group-item-heading"><a href="{url page="user" op="subscriptions"}"><span><i class="fa fa-check-square"></i></span> {translate key="user.manageMySubscriptions"}</a></li>
		{/if}
	{/if}
	{if $currentJournal}
		{if $acceptGiftPayments}
			<li class="list-group-item-heading"><a href="{url page="user" op="gifts"}"><span><i class="fa fa-fw fa-gift"></i></span> {translate key="gifts.manageMyGifts"}</a></li>
		{/if}
	{/if}
	<li class="list-group-item-heading"><a href="{url page="user" op="profile"}"><span><i class="glyphicon glyphicon-pencil"></i></span> {translate key="user.editMyProfile"}</a></li>

	{if !$implicitAuth}
		<li class="list-group-item-heading"><a href="{url page="user" op="changePassword"}"><span><i class="fa fa-lock"></i></span> {translate key="user.changeMyPassword"}</a></li>
	{/if}

	{if $currentJournal}
		{if $journalPaymentsEnabled && $membershipEnabled}
			{if $dateEndMembership}
				<li><a href="{url page="user" op="payMembership"}"><span><i class="glyphicon glyphicon-bell"></i></span> {translate key="payment.membership.renewMembership"}</a> ({translate key="payment.membership.ends"}: {$dateEndMembership|date_format:$dateFormatShort})</li>
			{else}
				<li><a href="{url page="user" op="payMembership"}"><span><i class="fa-bank"></i></span> {translate key="payment.membership.buyMembership"}</a></li>
			{/if}
		{/if}{* $journalPaymentsEnabled && $membershipEnabled *}
	{/if}{* $userJournal *}

	<li><a href="{url page="login" op="signOut"}"><span><i class="fa fa-sign-out"></i></span> {translate key="user.logOut"}</a></li>
	{call_hook name="Templates::User::Index::MyAccount"}
</ul>
</div>
</div>
</div>
<a href="{url op="customized"}">Customized Link</a>
