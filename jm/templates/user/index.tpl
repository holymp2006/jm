...
{if $isSiteAdmin}
	...
	<div class="panel panel-success">
		<div class="panel-heading">
			...
			<h4 class="panel-title"><strong><span><i class="fa fa-user"></i></span> {translate key=$isSiteAdmin->getRoleName()}</strong></h4></div>
			<p class="panel-body well">Click here to manage site settings, hosted journals, languages, system information, cache and logs</p>
		</a>
...
<div id="myJournals">
{if !$currentJournal}<h3><span><i class="fa fa-book"></i></span> {translate key="user.myJournals"}</h3>{/if}
					...
					<div class="panel panel-primary">
						<div class="panel-heading">
							<a href="{url journal=$journalPath page="manager"}">
								<h4 class="panel-title" style="color:#fff;"><strong><span><i class="fa fa-wrench"></i></span> {translate key="user.role.manager"}</strong></h4></div><p class="panel-body well">Click here to browse the file system, access journal sections, review forms, modify your journal setup, view stats and reports, manage users, roles, etc.</p>
							</a>
					</div>
				
				<div class="col-sm-12 alert alert-warning">{if $setupIncomplete.$journalId}<div>We've noticed that your journal setup hasn't been done yet or is incomplete.</div>Click <a href="{url journal=$journalPath page="manager" op="setup" path="1"}" class="btn btn-default hvr-sweep-to-bottom">{translate key="manager.setup"}</a>{/if} to finish off the process. It'll only take a few minutes.</div>		
