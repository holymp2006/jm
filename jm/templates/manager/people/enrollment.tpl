...
<form method="post" action="{url path=$roleSymbolic}">
	<div class="form-inline">
		...
		<input type="text" size="10" name="search" class="form-control" value="{$search|escape}" />
...
<form id="people" action="{url page="user" op="email"}" method="post">
	...
	<div id="users">
		...
		{if $userExists}
		<p><input type="submit" value="{translate key="email.compose"}" class="btn btn-primary"/>&nbsp;<input type="button" value="{translate key="common.selectAll"}" class="btn btn-default" onclick="toggleChecked()" />  <input type="button" value="{translate key="common.cancel"}" class="btn btn-warning" onclick="document.location.href='{url page="manager" escape=false}'" /></p>
		{/if}
	</div>
</form>
