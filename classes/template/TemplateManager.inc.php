...
class TemplateManager extends PKPTemplateManager {
	...
	function smartyPageLinks($params, &$smarty) {
		...
		if ($page>1) {
			$params[$paramName] = 1;
			$value .= '<li><a href="' . Request::url(null, null, null, Request::getRequestedArgs(), $params, $anchor, true) . '"' . $allExtra . '>&lt;&lt;</a></li>&nbsp;';
		...
		for ($i=$pageBase; $i<min($pageBase+$numPageLinks, $pageCount+1); $i++) {
			if ($i == $page) {
				...
				$params[$paramName] = $i;
				$value .= '<li><a href="' . Request::url(null, null, null, Request::getRequestedArgs(), $params, $anchor, true) . '"' . $allExtra . '>' . $i . '</a></li>&nbsp;';
			}
		}
		if ($page < $pageCount) {
			$params[$paramName] = $page + 1;
