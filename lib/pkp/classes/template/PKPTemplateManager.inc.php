...
class PKPTemplateManager extends Smarty {
	...
	function PKPTemplateManager($request = null) {
		...
		$this->over_template_dir = $baseDir . DIRECTORY_SEPARATOR . 'jm' . DIRECTORY_SEPARATOR . 'templates';
	...
	function smartyPageLinks($params, &$smarty) {
		...
		if ($page>1) {
			$params[$paramName] = 1;
			$value .= '<li><a href="' . $this->request->url(null, null, null, $this->request->getRequestedArgs(), $params, $anchor) . '"' . $allExtra . '>&lt;&lt;</a></li>&nbsp;';
		...
		for ($i=$pageBase; $i<min($pageBase+$numPageLinks, $pageCount+1); $i++) {
			if ($i == $page) {
				$value .= "<li><a><strong>$i</strong></a></li>&nbsp;";
			} else {
				$params[$paramName] = $i;
				$value .= '<li><a href="' . $this->request->url(null, null, null, $this->request->getRequestedArgs(), $params, $anchor) . '"' . $allExtra . '>' . $i . '</a></li>&nbsp;';
			}
		}
		if ($page < $pageCount) {
			$params[$paramName] = $page + 1;
