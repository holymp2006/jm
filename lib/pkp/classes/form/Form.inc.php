...
class Form {
	...
	function smartyFormLanguageChooser($params, &$smarty) {
		...
		$returner .= '<div id="languageSelector"><select size="1" name="formLocale" id="formLocale" class="form-control">';
		...
		$returner .= '</select><input type="submit" class="btn btn-primary" value="'. __('form.submit'). '" onclick="changeFormAction(\'' . htmlentities($params['form'], ENT_COMPAT, LOCALE_ENCODING) . '\', \'' . htmlentities($params['url'], ENT_QUOTES, LOCALE_ENCODING) . '\'); return false" /></div>';
		return $returner;
	}
